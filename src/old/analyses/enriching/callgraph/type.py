...
from ..mypyserver import PackageMypyServer
...
class CallsiteGetter(TraverserVisitor):
    def __init__(self, api: ApiCollection, result: Caller, resolver: FunctionResolver, logger: logging.Logger) -> None:
        super().__init__()
        self.api = api
            ...
            match o.callee:
                ...
                case MemberExpr() as member:
                    ...
                    if hasAnyType(exprTypes):
                        site.targets = self.resolver.resolveTargetsByName(member.name)
                    else:
                        targets = []
                        for tp in exprTypes:
