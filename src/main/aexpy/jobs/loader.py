...
class AnalysisResultLoader:
    ...
    def result(self, project:str, version: str) -> Tuple[ApiCollection | None, PayloadLog] | Exception:
        ...
        try:
            ...
            result = deserialize(resultPath.read_text()) if resultPath.exists() and resultPath.is_file() else None
            return result, log
        except Exception as ex:
            logging.error(f"Failed to load analysis {project} @ {version}", exc_info=ex)
            return ex
    
    def results(self, project: str | None) -> Iterable[Tuple[str, str, Tuple[ApiCollection | None, PayloadLog] | Exception]]:
        if project is None:
            ...
            for ver in self.versions(project):
                yield project, ver, self.result(project, ver)
