...
<template>
    <router-link :to="distributionUrl(prev)" custom v-slot="{ href, navigate }" v-if="prev">
        <n-button tag="a" :href="href" @click="navigate" type="info" ghost>
            <n-icon size="large" :component="PrevIcon" />
        </n-button>
    </router-link>
    <router-link :to="distributionUrl(succ)" custom v-slot="{ href, navigate }" v-if="succ">
        <n-button tag="a" :href="href" @click="navigate" type="info" ghost>
